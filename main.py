...
def _now_iso():
    # Return ISO with explicit Z (UTC)
    return datetime.now(timezone.utc).isoformat().replace("+00:00", "Z")

def _now_ts():
    # Return aware datetime in UTC for DB insertions
...
def _iso_to_dt(s: Optional[str]):
    """
    Convert stored ISO strings (with trailing Z or without) into timezone-aware datetimes (UTC).
    Returns None if s falsy or unparseable.
    """
    if not s:
        return None
    try:
        # If endswith Z → strip and parse, then attach UTC tzinfo
        if s.endswith("Z"):
            dt = datetime.fromisoformat(s[:-1])
            if dt.tzinfo is None:
                return dt.replace(tzinfo=timezone.utc)
            return dt.astimezone(timezone.utc)
        # Otherwise try parsing; if naive assume UTC
...
def _get_session_username_pg(conn, token: str) -> Optional[str]:
    with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
        ...
        # exp will be timezone-aware from DB; compare with timezone-aware now
            ...
            for t, info in list(sess.items()):
                ...
                try:
                    if exp and exp.endswith("Z"):
                        exp_dt = datetime.fromisoformat(exp[:-1]).replace(tzinfo=timezone.utc)
                    elif exp:
            ...
            try:
                ...
                if tr.amount is not None:
                    try:
                        ...
                        amt = 0.0
                # IMPORTANT: apply the balance change and persist to PG via _upsert_user_pg
                if tr.amount is not None:
                    if res == "win":
                        cur_user["balance"] = round(float(cur_user.get("balance", START_BALANCE)) + amt, 2)
                    else:
                        # subtract on loss — this updates the authoritative PG value and thus the leaderboard ordering
                        cur_user["balance"] = round(float(cur_user.get("balance", START_BALANCE)) - amt, 2)
                cur_user["last_update"] = _now_iso()
                # upsert user (authoritative write to PG)
            ...
            try:
                ...
                if tr.amount is not None:
                    if res == "win":
                        ...
                        # subtract on loss, persists to PG via upsert
                        u["balance"] = round(float(u.get("balance", START_BALANCE)) - amt, 2)
                u["last_update"] = _now_iso()
                _upsert_user_pg(conn, user_key, u)
                trade_entry = {
